...
/**
 * Query to fetch a single playlist document by slug from Sanity
 * 
 * Conditions:
 * - Document type must be "playlist"
 * - Matches exact slug.current parameter
 * - Returns first (and should be only) matching document
 * 
 * Returns playlist details including: Internal Sanity ID (_id), Title, URL slug
 * - Selected startups array containing startup details like: Internal ID, Creation date, Title, Slug
 * - Each startup includes author details (ID, name, slug, image, bio), view count, description, category, featured image, pitch
 */
